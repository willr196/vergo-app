...
export const jobsApi = {
  ...
  async getJobs(filters?: JobFilters, page = 1, limit = 20): Promise<PaginatedJobsResponse> {
    ...
    const response = await apiClient.get<any>(`/api/v1/jobs?${params.toString()}`);
    
    // Backend returns { jobs, pagination } directly
    return {
      ok: true,
  ...
  async getJob(jobId: string): Promise<Job> {
    const response = await apiClient.get<BackendResponse<Job>>(`/api/v1/jobs/${jobId}`);
  ...
  async createJob(jobData: Partial<Job>): Promise<Job> {
    ...
    if (response.data.ok && (response.data.job || response.data.data)) {
      return response.data.job || response.data.data!;
    }
